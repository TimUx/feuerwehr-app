    ...
    if ($step == 0) {
        ...
        if (empty($errors)) {
            ...
            $config = [
                ...
                // Note: data_dir and backup_dir will be added manually to preserve __DIR__
            ...
            // Generate PHP config file content manually to preserve __DIR__ references
            // We can't use var_export() because it would evaluate __DIR__ to an absolute path
            $configContent = "<?php\n";
            $configContent .= "/**\n";
            $configContent .= " * Configuration file for Feuerwehr App\n";
            $configContent .= " * Generated by Installation Wizard on " . date('Y-m-d H:i:s') . "\n";
            $configContent .= " * \n";
            $configContent .= " * IMPORTANT: The 'data_dir' and 'backup_dir' paths use __DIR__ to ensure\n";
            $configContent .= " * they work correctly regardless of where the application is installed.\n";
            $configContent .= " * Do not change these to absolute paths!\n";
            $configContent .= " */\n\n";
            $configContent .= "return [\n";
            $configContent .= "    // Application settings\n";
            $configContent .= "    'app_name' => " . var_export($config['app_name'], true) . ",\n";
            $configContent .= "    'app_version' => " . var_export($config['app_version'], true) . ",\n";
            $configContent .= "    \n";
            $configContent .= "    // Security settings\n";
            $configContent .= "    'encryption_key' => " . var_export($config['encryption_key'], true) . ",\n";
            $configContent .= "    'session_lifetime' => " . var_export($config['session_lifetime'], true) . ",\n";
            $configContent .= "    \n";
            $configContent .= "    // Email settings for form submissions\n";
            $configContent .= "    'email' => [\n";
            $configContent .= "        'from_address' => " . var_export($config['email']['from_address'], true) . ",\n";
            $configContent .= "        'from_name' => " . var_export($config['email']['from_name'], true) . ",\n";
            $configContent .= "        'to_address' => " . var_export($config['email']['to_address'], true) . ",\n";
            $configContent .= "        'smtp_host' => " . var_export($config['email']['smtp_host'], true) . ",\n";
            $configContent .= "        'smtp_port' => " . var_export($config['email']['smtp_port'], true) . ",\n";
            $configContent .= "        'smtp_auth' => " . var_export($config['email']['smtp_auth'], true) . ",\n";
            $configContent .= "        'smtp_username' => " . var_export($config['email']['smtp_username'], true) . ",\n";
            $configContent .= "        'smtp_password' => " . var_export($config['email']['smtp_password'], true) . ",\n";
            $configContent .= "        'smtp_secure' => " . var_export($config['email']['smtp_secure'], true) . ",\n";
            $configContent .= "    ],\n";
            $configContent .= "    \n";
            $configContent .= "    // Data directory paths (relative to config directory)\n";
            $configContent .= "    'data_dir' => __DIR__ . '/../data',\n";
            $configContent .= "    'backup_dir' => __DIR__ . '/../data/backups',\n";
            $configContent .= "    \n";
            $configContent .= "    // Default admin credentials (used as fallback if users.json is deleted)\n";
            $configContent .= "    'default_admin' => [\n";
            $configContent .= "        'username' => " . var_export($config['default_admin']['username'], true) . ",\n";
            $configContent .= "        'password' => " . var_export($config['default_admin']['password'], true) . ",\n";
            $configContent .= "    ],\n";
            $configContent .= "];\n";
